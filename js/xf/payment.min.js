...
XF.StripePaymentForm=XF.Element.newHandler({options:{publishableKey:null,formStyles:".js-formStyles",prEnabled:null,prCountry:null,prCost:null,prCurrency:null,prLabel:null,styleType:"dark"},stripe:null,elements:null,elementsCache:{},paymentRequest:null,stripeJs:"https://js.stripe.com/v3/",processing:null,init:function(){this.$target.on("submit",XF.proxy(this,"submit"));XF.loadedScripts.hasOwnProperty(this.stripeJs)?this.postInit():XF.loadScript(this.stripeJs,XF.proxy(this,"postInit"))},postInit:function(){if(this.options.publishableKey){this.stripe=
Stripe(this.options.publishableKey);this.elements=this.stripe.elements();this.options.prEnabled&&(this.paymentRequest=this.stripe.paymentRequest({country:this.options.prCountry,currency:this.options.prCurrency.toLowerCase(),total:{label:this.options.prLabel,amount:this.options.prCost},requestPayerName:!0,requestPayerEmail:!0}));this.initElements();var b=this.$target.closest(".overlay-container").data("overlay");b.on("overlay:hidden",function(){b.destroy();a("iframe").each(function(){var b=a(this);
0<=b.attr("name").toLowerCase().indexOf("stripe")&&b.remove()})})}else console.error("Form must contain a data-publishable-key attribute.")},initElements:function(){var b=this,c=this.elements,d=this.paymentRequest,e=this.$target.find(this.options.formStyles)||{};e=e?a.parseJSON(e.first().html()):{};if(d){var f=c.create("paymentRequestButton",{paymentRequest:d,style:{paymentRequestButton:{theme:"light"===this.options.styleType?"light-outline":"dark"}}});d.canMakePayment().then(function(a){a?(f.mount("#payment-request-button"),
b.$target.find(".js-pr-remove").show()):b.$target.find(".js-pr-remove").remove()});d.on("token",function(a){b.stripeTokenHandler(a.token)})}d=c.create("cardNumber",{style:e});d.mount("#card-number-element");this.elementsCache.cardNumber=d;d.on("change",function(b){var c=b.brand,d={visa:"fa-cc-visa",mastercard:"fa-cc-mastercard",amex:"fa-cc-amex",discover:"fa-cc-discover",diners:"fa-cc-diners",jcb:"fa-cc-jcb",unknown:"fa-credit-card-alt"};b=a("#brand-icon");var e="fa-credit-card";b[0].className="fa--xf fa--inputOverlay";
c&&"unknown"!==c?c in d?(e=d[c],c="fab",d="fa"+XF.config.fontAwesomeWeight):(c="fa"+XF.config.fontAwesomeWeight,d="fab"):(c="fa"+XF.config.fontAwesomeWeight,d="fab");b.addClass(c);b.removeClass(d);b.addClass(e)});d=c.create("cardExpiry",{style:e});d.mount("#card-expiry-element");this.elementsCache.cardExpiry=d;c=c.create("cardCvc",{style:e});c.mount("#card-cvc-element");this.elementsCache.cardCvc=c},getTokenData:function(b){var c={};b=this.$target.find("[data-token-data]");b.length&&b.each(function(){var b=
a(this);c[b.data("token-data")]=b.val()});return c},submit:function(b){b.preventDefault();if(this.processing)return!1;this.processing=!0;var c=this;b=a(b.target);var d=this.stripe,e=this.elementsCache.cardNumber,f=a("#card-errors-container"),g=f.find("#card-errors");b.addClass("is-disabled").prop("disabed",!0);d.createToken(e,this.getTokenData()).then(function(a){a.error?(g.text(a.error.message),g.removeClass("u-hidden"),f.removeClass("u-hidden"),c.processing=!1):(f.addClass("u-hidden"),g.addClass("u-hidden"),
g.text(""),c.stripeTokenHandler(a.token))})},stripeTokenHandler:function(b){var c=this.$target,d=a('<input type="hidden" />');d.attr("name","response[id]");d.attr("value",b.id);c.append(d);this.response()},response:function(){var a=this.$target,c=XF.getDefaultFormData(a);XF.ajax("post",a.attr("action"),c,XF.proxy(this,"complete"),{skipDefaultSuccess:!0})},complete:function(a){this.processing=!1;a.redirect&&XF.redirect(a.redirect)}});XF.Element.register("payment-provider-container","XF.PaymentProviderContainer");
XF.Element.register("braintree-payment-form","XF.BraintreePaymentForm");XF.Element.register("braintree-apple-pay-form","XF.BraintreeApplePayForm");XF.Element.register("braintree-paypal-form","XF.BraintreePayPalForm");XF.Element.register("stripe-payment-form","XF.StripePaymentForm")}(jQuery,window,document);
