...
class ApprovalQueue extends Repository
{
	...
	public function rebuildUnapprovedCounts()
	{
		...
		// reflect changes in the rebuilt cache immediately (mostly for the UI)
		$cache['lastBuilt'] = $cache['lastModified'];
		$sessionCache = $cache;
		unset($sessionCache['lastModified']);
		$session = $this->app()->session();
		$existing = $session->unapprovedCounts;
		$session->unapprovedCounts = array_replace($existing ?: [], $sessionCache);
